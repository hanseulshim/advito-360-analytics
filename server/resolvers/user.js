...
import { saltHash, generateAccessToken, getExpirationDate, sendEmail, checkValidPassword, validateEmail } from '../util'
...
export default {
  Query: {
    getUser: async (_, { id }) => {
      ...
      const roleIds = await user.$relatedQuery('advitoUserRoleLink').map(role => role.advitoRoleId)
      return {
        ...user,
        roleIds
      }
  ...
  Mutation: {
    login: async (_, { username, password }) => {
      ...
      const roleIds = await user.$relatedQuery('advitoUserRoleLink').map(role => role.advitoRoleId)
      const session = await user.$relatedQuery('advitoUserSession').where('sessionEnd', null).first()
      const sessionToken = crypto.randomBytes(16).toString('base64')
      if (session) await user.$relatedQuery('advitoUserSession').patch({ sessionEnd: new Date() }).where('sessionEnd', null)
    ...
    logout: async (_, { sessionToken }) => {
      const session = await AdvitoUserSession.query().where('sessionToken', sessionToken).where('sessionEnd', null).first()
      if (!session) throw new AuthenticationError('User session not found')
      await AdvitoUserSession.query().patch({ sessionEnd: new Date() }).where('sessionToken', sessionToken).where('sessionEnd', null)
      return true
    },
    sendResetPasswordEmail: async (_, { email }) => {
      const user = await AdvitoUser.query().where('email', email).first()
      if (!user) throw new UserInputError('User not found')
      const oldToken = await user.$relatedQuery('accessToken').where('isActive', true).first()
      if (oldToken) await user.$relatedQuery('accessToken').patch({ isActive: false }).where('isActive', true).first()
    ...
    createUser: async (_, {
      ...
      const checkUser = await AdvitoUser.query().where('username', email).first()
