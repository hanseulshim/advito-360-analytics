...
export const authenticateUser = async (sessionToken) => {
  const session = await AdvitoUserSession.query().where('sessionToken', sessionToken)
    .andWhere('sessionEnd', null).first()
  ...
  if (timeDifference > 50) {
    await AdvitoUserSession
      .patchAndFetchById(id, { sessionExpiration: newExpiration })
  }

  const user = await session
    .$relatedQuery('advitoUser')
    .first()
